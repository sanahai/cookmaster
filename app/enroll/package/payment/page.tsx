    ...
    <>
      ...
      <main className="mx-auto max-w-2xl px-4 py-10">
        ...
        <p className="mb-8 text-beauty-gray">
          조리기능사 패키지(4종 전체) 신청이 접수되었습니다. 아래 계좌로 입금 후{" "}
          <strong className="text-beauty-neutral">관리자 승인</strong> 시 4개 과정이 모두 활성화됩니다.
        </p>

        {/* 결제 금액 */}
        <div className="mb-5 rounded-card bg-white p-6 text-center shadow-card">
          <p className="text-sm text-beauty-gray">결제 금액 (패키지 4종)</p>
          <p className="mt-1 text-4xl font-extrabold text-primary">
            {PACKAGE_PRICE.toLocaleString()}
            <span className="ml-1 text-2xl">원</span>
          </p>
          <p className="mt-2 text-sm text-beauty-neutral">
            {courses.map((c) => c.name.replace(" 필기", "")).join(" · ")}
          </p>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary to-primary-light p-6 text-white shadow-cardHover">
          ...
          <div className="relative">
            <div className="mb-6 flex items-center justify-between">
              ...
              <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-bold">PACKAGE</span>
            </div>
        ...
        <div className="mt-6 rounded-card bg-white p-5 text-sm shadow-card">
          ...
          <ul className="mt-2 list-disc space-y-1.5 pl-5 text-beauty-gray">
            ...
            <li>관리자가 입금을 확인하고 승인하면 4개 과정이 모두 활성화됩니다.</li>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <Link href="/enroll" className="btn-outline flex-1 text-center">
            수강신청으로 돌아가기
          </Link>
        </div>
      </main>
